...
export const getTrendingMovies = async () => {
  const { data } = await tmdbClient.get<ApiResponse<Movie>>(
    "/trending/movie/week",
  );
  return data;
};
...
export const getMovieWatchProviders = async (movieId: string) => {
  const { data } = await tmdbClient.get<WatchProvidersResponse>(
...
export const getPersonMovieCredits = async(personId:string)=>{
  const {data} =await tmdbClient.get(`/person/${personId}/combined_credits`)
  return data
}
